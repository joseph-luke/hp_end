...
m_mod <- readRDS(paste(path, "Processed/am_mod.rda", sep=""))

fit_df <- read_csv(paste(path, "Processed/fit_am.csv", sep=""), show_col_types = FALSE)
test_df <- read_csv(paste(path, "Processed/predict_am.csv", sep=""), show_col_types = FALSE)
...
write_csv(prediction_df, paste(path, "Processed/predicted_am.csv", sep=""))
